...
const LandingPage: React.FC<LandingPageProps> = ({ onInitialize }) => {
  ...
  const technicalTerms = [
    'KERNEL_INIT', 'GRADIENT_STABLE', 'BACKPROP_LINKED', 'TENSOR_FLOW_ACTIVE',
    'NUMPY_BLAS_OK', 'RESNET_ARCH_9.5', 'AURA_CORE_V3', 'UPLINK_SYNCED'
  ];

  useEffect(() => {
    // High-impact cinematic entry
    const runBoot = async () => {
  ...
  if (bootStatus !== 'READY') {
    return (
      <div className="fixed inset-0 z-[2000] bg-black flex flex-col items-center justify-center overflow-hidden">
        <div className={`absolute inset-0 bg-white opacity-0 ${bootStatus === 'BREACH' ? 'animate-flash' : ''}`} />
        <div className="relative z-10 flex flex-col items-center gap-12 text-center">
          ...
          <div className="space-y-4">
             <div className="text-[11px] font-black text-white/40 uppercase tracking-[1em] ml-[1em] animate-pulse">Establishing Nexus</div>
             <div className="text-4xl md:text-5xl font-black text-white italic tracking-tighter">{activeTerm}</div>
          </div>
        </div>
        {/* Grain Noise */}
        <div className="absolute inset-0 pointer-events-none opacity-[0.03] bg-[url('https://media.giphy.com/media/oEI9uWUic9VKM/giphy.gif')] mix-blend-screen" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#010208] text-white selection:bg-indigo-500/40 overflow-x-hidden relative animate-in zoom-in fade-in duration-1000">
      ...
      <nav className={`fixed top-0 w-full z-[100] transition-all duration-700 ${scrollY > 60 ? 'bg-black/95 backdrop-blur-3xl border-b border-white/5 py-5 shadow-2xl shadow-indigo-500/10' : 'py-12'}`}>
        <div className="max-w-[1700px] mx-auto px-12 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-16 text-[11px] font-black uppercase tracking-[0.5em] text-slate-500">
            {['Applied Architecture', 'Neural Audit', 'Pathfinder', 'Join'].map((item) => (
              <a key={item} href={`#${item.toLowerCase().replace(' ', '-')}`} className="hover:text-white transition-colors relative group">
                {item}
                <span className="absolute -bottom-1 left-0 w-0 h-px bg-white transition-all group-hover:w-full"></span>
              </a>
            ))}
            <button onClick={onInitialize} className="px-12 py-4 bg-white text-black rounded-full font-black text-[12px] hover:bg-indigo-50 transition-all active:scale-95 shadow-2xl">LAUNCH TERMINAL</button>
          </div>
        </div>
      </nav>
      ...
      <section className="relative min-h-screen w-full flex flex-col items-center justify-center px-12 z-10 pt-24">
        <div className="max-w-7xl w-full text-center space-y-16">
          <div className={`inline-flex items-center gap-6 px-10 py-4 rounded-full glass border border-white/10 shadow-3xl transition-all duration-1000 delay-100 ${visibleElements.has('hero-badge') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'} reveal-trigger`} id="hero-badge">
            ...
            <span className="text-[13px] font-black uppercase tracking-[0.6em] text-indigo-400">PURE APPLIED INTELLIGENCE</span>
          </div>
          ...
          <div className={`space-y-12 transition-all duration-1000 delay-300 ${visibleElements.has('hero-title') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'} reveal-trigger`} id="hero-title">
            <h1 className="text-7xl md:text-9xl lg:text-[14rem] font-black text-white tracking-[0.02em] leading-[0.8] uppercase italic">
              Architect <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-violet-400 to-emerald-400 drop-shadow-2xl">The Core.</span>
            </h1>
          </div>
          ...
          <p className={`text-2xl md:text-4xl text-slate-400 max-w-6xl mx-auto leading-tight font-medium transition-all duration-1000 delay-500 ${visibleElements.has('hero-sub') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'} reveal-trigger`} id="hero-sub">
            Passive learning is for spectators. <br />
            <span className="text-white italic">Become a Technical Architect.</span> Master industrial-grade Python engineering through deep applied benchmarks.
          </p>
          ...
          <div className={`flex flex-col sm:flex-row gap-12 justify-center pt-12 transition-all duration-1000 delay-700 ${visibleElements.has('hero-cta') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'} reveal-trigger`} id="hero-cta">
            ...
            <button onClick={() => document.getElementById('architecture')?.scrollIntoView({behavior:'smooth'})} className="px-24 py-12 bg-slate-900/40 border-2 border-slate-800 text-white rounded-[48px] font-black text-5xl backdrop-blur-3xl hover:bg-slate-800 hover:border-indigo-500/50 transition-all active:scale-95 shadow-2xl uppercase tracking-tighter italic">Nodes</button>
          </div>
        </div>
      </section>

      {/* Applied Grid */}
      <section id="architecture" className="w-full py-72 px-12 relative z-10 bg-[#010208]">
        <div className="max-w-[1600px] mx-auto space-y-64">
          <div className={`text-center space-y-14 transition-all duration-1000 ${visibleElements.has('arch-head') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'} reveal-trigger`} id="arch-head">
            <h2 className="text-7xl md:text-[10rem] font-black tracking-tighter text-white uppercase leading-none italic">The Architecture.</h2>
            <p className="text-2xl lg:text-3xl text-slate-500 font-medium max-w-6xl mx-auto leading-relaxed uppercase tracking-[0.4em]">
              The PyQuest repository is optimized for depth. From math primitives to high-fidelity transformer logic.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            {appliedNodes.map((c, i) => (
      ...
      {/* Neural Audit USP */}
      <section id="neural-audit" className="w-full py-72 px-12 relative z-10 overflow-hidden bg-gradient-to-b from-transparent via-indigo-950/[0.05] to-transparent">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-48 items-center">
          <div className={`space-y-24 transition-all duration-1000 ${visibleElements.has('audit-text') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-32'} reveal-trigger`} id="audit-text">
            <h2 className="text-8xl lg:text-[12rem] font-black tracking-tighter text-white uppercase leading-[0.7] italic">Neural <br /> Audit.</h2>
            <p className="text-3xl text-slate-400 leading-relaxed font-medium">
              We verify structural mathematical integrity. Every logical kernel authored on PyQuest is audited by <span className="text-white italic">Aura</span> in real-time.
            </p>
            <div className="space-y-16">
              {[
                ...
                { title: 'Strategic Resource Mapping', desc: 'Aura-guided mentoring on production architectures.' }
              ].map((usp, idx) => (
          ...
          <div className={`relative transition-all duration-1000 ${visibleElements.has('audit-visual') ? 'opacity-100 scale-100 translate-x-0' : 'opacity-0 scale-75 translate-x-32'} reveal-trigger`} id="audit-visual">
            ...
            <div className="relative bg-[#0b0e14] border border-white/10 rounded-[80px] p-24 shadow-3xl backdrop-blur-3xl overflow-hidden group hover:border-indigo-500/30 transition-all duration-700">
               <div className="space-y-16">
                 ...
                 <h4 className="text-5xl font-black text-white uppercase tracking-tighter italic">Audit Protocol v9.5.3</h4>
                 <div className="p-14 bg-[#010208] border border-white/5 rounded-[64px] font-mono text-2xl space-y-10 relative overflow-hidden">
                    <div className="text-indigo-500 text-xs font-black uppercase tracking-[0.5em] mb-4">KERNEL_ACTIVE</div>
                    ...
                    <div className="mt-14 p-10 bg-emerald-500/10 border border-emerald-500/20 rounded-[40px] animate-in slide-in-from-top-4">
                      <div className="text-emerald-400 text-xs font-black uppercase tracking-widest mb-4">Verdict: Sound</div>
                      <p className="text-lg text-emerald-200/80 leading-relaxed">Jacobian product verified. Memory stride aligned with tensor layout.</p>
                    </div>
                 </div>
               </div>
            </div>
      ...
      {/* Pathfinder */}
      <section id="pathfinder" className="w-full py-72 px-12 relative z-10 bg-[#020410]">
        <div className="max-w-[1600px] mx-auto text-center space-y-48">
          <div className={`space-y-14 transition-all duration-1000 ${visibleElements.has('path-head') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-32'} reveal-trigger`} id="path-head">
            <h2 className="text-8xl md:text-[11rem] font-black tracking-tighter text-white uppercase leading-none italic">The Pathfinder.</h2>
            <p className="text-2xl text-slate-500 font-medium max-w-5xl mx-auto uppercase tracking-widest leading-relaxed">
              Define your objective—Quant Researcher, Robotics Lead, or Founder—and PyQuest synthesizes your tech-tree in real-time.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 reveal-trigger" id="path-grid">
             {['Quant Researcher', 'Neural Architect', 'Inference Lead', 'Robotics Architect'].map((role, i) => (
               <div key={i} className={`p-16 bg-[#0b0e14] border border-white/5 rounded-[64px] hover:bg-white hover:text-black transition-all cursor-pointer group shadow-3xl hover:-translate-y-10 duration-700 ${visibleElements.has('path-grid') ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-32 scale-90'}`} style={{ transitionDelay: `${i * 100}ms` }}>
                 ...
                 <div className="text-4xl font-black text-white group-hover:text-black uppercase tracking-tighter group-hover:scale-110 transition-transform italic">{role}</div>
               </div>
             ))}
          </div>
        </div>
      ...
      {/* Final CTA */}
      <section id="join" className="w-full py-80 px-12 text-center relative z-10">
        <div className="max-w-[1600px] mx-auto">
          <div className={`relative bg-white rounded-[140px] p-64 md:p-80 shadow-[0_60px_150px_-20px_rgba(255,255,255,0.2)] overflow-hidden group transition-all duration-1000 ${visibleElements.has('final-cta') ? 'opacity-100 scale-100' : 'opacity-0 scale-90'} reveal-trigger`} id="final-cta">
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-600 to-indigo-950 transition-opacity duration-1000 opacity-0 group-hover:opacity-100"></div>
            <div className="relative z-10 space-y-24">
              <h2 className="text-8xl md:text-[16rem] font-black text-slate-950 tracking-tighter uppercase leading-[0.7] group-hover:text-white transition-colors duration-700 italic">
                Initialize <br /> Now.
              </h2>
              <p className="text-3xl text-slate-600 font-medium max-w-5xl mx-auto leading-relaxed group-hover:text-indigo-50 transition-colors duration-700">
                Join 10,000+ technical architects engineering the future of intelligence. Pure Python. Zero fluff. Applied industrial architecture.
              </p>
              <div className="pt-24">
                <button 
                  onClick={onInitialize}
                  className="px-72 py-24 bg-slate-950 text-white rounded-[80px] font-black text-7xl shadow-3xl hover:scale-[1.05] active:scale-95 transition-all uppercase tracking-tighter group-hover:bg-white group-hover:text-indigo-700 duration-700 italic"
                >
                  Enter Academy
                </button>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-12 text-center space-y-32">
          <div className="flex flex-col items-center gap-12">
            ...
            <p className="text-[14px] font-black text-slate-800 uppercase tracking-[1em]">
              &copy; 2026 PYQUEST PLATFORMS INC. 9.5.3_STABLE_REBUILD
            </p>
          </div>
        </div>
      </footer>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .animate-flash { animation: flash 0.3s ease-in-out forwards; }
