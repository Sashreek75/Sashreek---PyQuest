...
const AuraHub: React.FC<AuraHubProps> = ({ isOpen, onOpen, onClose, context, personalization }) => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const auraResponse = await chatWithAura(userMsg, context, personalization);
  ...
  // Fixed the floating button logic to ensure it shows when closed
  return (
    <>
      {!isOpen && (
        <button 
          onClick={onOpen}
          className="fixed bottom-10 right-10 w-20 h-20 bg-indigo-600 rounded-[24px] flex items-center justify-center text-white shadow-[0_0_50px_rgba(79,70,229,0.4)] hover:scale-110 active:scale-95 transition-all z-[400] group"
        >
